...
| Solid 2 / Solid 1 canvas addon                                                 | `./renderer/solid2`, `./renderer/solid1`                   |
...
`solid-js-legacy` is rewritten to `solid-js` in `dist` after emit (dev alias for Solid 1, including subpaths like `/store` and `/web`). `neverBundle` lists that alias and the root self-imports `storybook-solidjs-vite/renderer/solid1` and `storybook-solidjs-vite/renderer/preview-addon`; peers and `dependencies` are already external.
...
- `src/renderer/preview-addon.ts` — default re-export of `solid2`
